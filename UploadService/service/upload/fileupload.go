...
func NewFileUploadService(bucket string) *FileUploadService {
	...
	return &FileUploadService{client, bucketName, logger}
}

func (u FileUploadService) UploadFile(file *multipart.FileHeader, name string) (string, error) {
	ctx := context.Background()
	f, err := file.Open()
	if err != nil {
...
func (u FileUploadService) ReplaceFile(file string, newFile *multipart.FileHeader) (string, error) {
	ctx := context.Background()
...
func (u FileUploadService) DeleteFile(file string) error {
	ctx := context.Background()
